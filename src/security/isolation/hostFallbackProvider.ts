...
export class HostFallbackEnvironment implements IIsolationEnvironment {
  readonly id: string;
  readonly providerName = 'host-fallback-provider';
  ...
  async execute(
    ...
    if (!this.alive) {
      throw new Error(`HostFallbackEnvironment ${this.id} is destroyed.`);
    }
...
export class HostFallbackProvider implements IIsolationProvider {
  readonly name = 'host-fallback-provider';
  /**
   * HostFallbackProvider reports PROCESS level only.
   * It MUST NOT satisfy requirements for FILESYSTEM, NETWORK, CONTAINER, or FULL.
   */
  readonly isolationLevel: IsolationLevel = 'PROCESS';
  ...
  async createEnvironment(
    ...
    const envId = `host_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
    return new HostFallbackEnvironment(envId, policy, capability.workspaceRoot);
  }
}
